...
All instructions (except for Curve4) and basic usage have been implemented, this includes:
...
- Around
  ...
  - Around(D)
- Colour
- ColourGradient - Stubbed, not yet implemented
- Curve3
...
- Dot-art images can be used to generate frames in a light show. The image requires a transparent background. The code will detect clumps of non-transparent pixels, average the colours, then return all the positions in a dictionary where keys are positions and values are the averaged colours. The relative size of dots will determine depth.
